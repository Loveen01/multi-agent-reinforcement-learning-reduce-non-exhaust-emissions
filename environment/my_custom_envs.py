...
from environment.helper_functions import get_delta_accum_waiting_time

import sys 
from datetime import datetime

sys.path.append('/Users/loveen/Desktop/Masters project/rl-multi-agent-traffic-nonexhaust-emissions')
from environment.helper_functions import get_total_waiting_time, get_tyre_pm, get_delta_waiting_time
...
class SumoEnvironmentCountAllRewards(SumoEnvironment):
    ...
    def _sumo_step(self):
        ...
        if self.single_eval_mode:
            ...
            self.log_tensor_step_metrics(system_stats=system_stats,
    ...
    def compute_agent_metrics(self):
        ...
        average_speed = [self.traffic_signals[ts].get_average_speed() for ts in self.ts_ids]
        abs_accelerations = [get_tyre_pm(ts_object) for ts_object in self.traffic_signals.values()]

        waiting_time = [get_total_waiting_time(ts_object) for ts_object in self.traffic_signals.values()]
        ...
        for i, ts in enumerate(self.ts_ids):
            ...
            agent_stats[f"{ts}_waiting_time"] = waiting_time[i]
                            
        return agent_stats

    def log_csv_step_metrics(self, system_stats, agent_stats):
        '''
        log metrics to csv file. If data is none, then plot all zeros.
        Note that it assumes preservation of insertion order of the data, 
        as the data headers have been preconfigured before 
        '''
...
class SumoEnvironmentPZCountAllRewards(SumoEnvironmentPZ):
    ...
    def __init__(self, tb_writer = None, 
        ...
        self.env = SumoEnvironmentCountAllRewards(**self._kwargs, 
                                                  ...
                                                  csv_path=csv_path,
                                                  tb_writer=tb_writer)  # instead of SumoEnvironment. CountAllRewardsEnv is subclass of SumoEnv  
    ...
    def step(self, action):
        ...
        if self.truncations[self.agent_selection] or self.terminations[self.agent_selection]:
            print(self.agent_selection, self.truncations[self.agent_selection], self.terminations[self.agent_selection] )
