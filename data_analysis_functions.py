...
def get_df_from_csv(path_to_checkpoint_eval_data, csv_file_name, eval_dir, seed):
...
def generate_summary_df_from_csv(path_to_checkpoint_eval_data, eval_dir:str, df_row_label, seed):
    """
        Collects and processes evaluation data from a CSV file into a summary DataFrame. for 2x2grid only
    ...
    """
    eval_metrics_df = get_df_from_csv(path_to_checkpoint_eval_data, eval_dir=eval_dir, seed=seed, csv_file_name="eval_metrics.csv")
    ...
    formatted_df.loc[df_row_label, "1_abs_accel_sum"] = eval_metrics_df["1_abs_accel"].sum()
    formatted_df.loc[df_row_label, "2_abs_accel_sum"] = eval_metrics_df["2_abs_accel"].sum()
    formatted_df.loc[df_row_label, "5_abs_accel_sum"] = eval_metrics_df["5_abs_accel"].sum()
    formatted_df.loc[df_row_label, "6_abs_accel_sum"] = eval_metrics_df["6_abs_accel"].sum()
    
    formatted_df.loc[df_row_label, "1_abs_accel_mean"] = eval_metrics_df["1_abs_accel"].mean()
    formatted_df.loc[df_row_label, "2_abs_accel_mean"] = eval_metrics_df["2_abs_accel"].mean()
    formatted_df.loc[df_row_label, "5_abs_accel_mean"] = eval_metrics_df["5_abs_accel"].mean()
    formatted_df.loc[df_row_label, "6_abs_accel_mean"] = eval_metrics_df["6_abs_accel"].mean()

    formatted_df.loc[df_row_label, "1_accumulated_waiting_time_mean"] = eval_metrics_df["1_accumulated_waiting_time"].mean()
    formatted_df.loc[df_row_label, "2_accumulated_waiting_time_mean"] = eval_metrics_df["2_accumulated_waiting_time"].mean()
    formatted_df.loc[df_row_label, "5_accumulated_waiting_time_mean"] = eval_metrics_df["5_accumulated_waiting_time"].mean()
    formatted_df.loc[df_row_label, "6_accumulated_waiting_time_mean"] = eval_metrics_df["6_accumulated_waiting_time"].mean()

    formatted_df.loc[df_row_label, "1_accumulated_waiting_time_sum"] = eval_metrics_df["1_accumulated_waiting_time"].sum()
    formatted_df.loc[df_row_label, "2_accumulated_waiting_time_sum"] = eval_metrics_df["2_accumulated_waiting_time"].sum()
    formatted_df.loc[df_row_label, "5_accumulated_waiting_time_sum"] = eval_metrics_df["5_accumulated_waiting_time"].sum()
    formatted_df.loc[df_row_label, "6_accumulated_waiting_time_sum"] = eval_metrics_df["6_accumulated_waiting_time"].sum()
...
def generate_summary_df_from_csv_for_4x4grid_resco(path_to_checkpoint_eval_data, eval_dir:str, df_row_label, seed):
    """
        Collects and processes evaluation data from a CSV file into a summary DataFrame. for 2x2grid only
    ...
    # # sum up the agent specific metrics
    # tot_agent_accel_vec = eval_metrics_df[f"{agent_ids[0]}_abs_accel"] + \
    #                       eval_metrics_df[f"{agent_ids[4]}_abs_accel"] + \
    #                       eval_metrics_df[f"{agent_ids[8]}_abs_accel"] + \
    #                       eval_metrics_df[f"{agent_ids[12]}_abs_accel"]
    
    # tot_agent_stopped = eval_metrics_df[f"{agent_ids[0]}_stopped"] + \
    #                     eval_metrics_df[f"{agent_ids[4]}_stopped"] + \
    #                     eval_metrics_df[f"{agent_ids[8]}_stopped"] + \
    #                     eval_metrics_df[f"{agent_ids[12]}_stopped"] 
    
    tot_agent_accum_wait_time = pd.concat([eval_metrics_df[f"{id}_waiting_time"] for id in agent_ids])
    tot_agent_accum_wait_time_sum = tot_agent_accum_wait_time.groupby(tot_agent_accum_wait_time.index).mean().sum()
    ...
    sim_time = eval_metrics_df['sim_time'].max()


    # similar as well - 
    formatted_df.loc[df_row_label, "sys_abs_accel_episode_mean"] = sys_abs_accel_vec.mean()
    formatted_df.loc[df_row_label, "sys_abs_accel_episode_mean_var"] = sys_abs_accel_vec.var()/sim_time

    formatted_df.loc[df_row_label, "sys_abs_accel_episode_rms"] = np.sqrt(np.mean(np.square(sys_abs_accel_vec)))
    formatted_df.loc[df_row_label, "sys_abs_accel_episode_var"] = sys_abs_accel_vec.var()
    formatted_df.loc[df_row_label, "sys_abs_accel_episode_rms_with_integ"] = np.sqrt(np.trapz(np.square(sys_abs_accel_vec))/sim_time)
    ...
    formatted_df.loc[df_row_label, "sys_total_stopped_episode_mean"] = eval_metrics_df["sys_total_stopped"].mean()
    # waiting time 
    formatted_df.loc[df_row_label, "sys_avg_waiting_time_total_episode_mean"] = eval_metrics_df["sys_total_wait"].mean()
    formatted_df.loc[df_row_label, "sys_avg_waiting_time_episode_mean"] = eval_metrics_df["sys_avg_wait"].mean()
    ...
    formatted_df.loc[df_row_label, "tot_agent_accum_wait_time"] = tot_agent_accum_wait_time_sum
...
def get_df_from_csv_name(path_to_checkpoint_eval_data, eval_dir, seed_folder_name):
